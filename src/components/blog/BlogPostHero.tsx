...
import { getBlogImageMeta } from "@/lib/blog/batch2ImageMeta";
...
export function BlogPostHero({
  ...
  const alt = heroMeta?.alt ?? post.coverImageAlt ?? post.title;
        ...
        <div className="relative overflow-hidden md:rounded-lg">
          {post.coverImageUrl ? (
            <MediaFrame
              ...
              alt={alt}
              title={heroMeta?.title_attr}
              width={heroMeta?.width ?? 1600}
              height={heroMeta?.height ?? 1000}
              priority
              fetchPriority="high"
              sizes={heroMeta?.sizes ?? "100vw"}
              unoptimized
